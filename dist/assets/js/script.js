...
jQuery(function ($) {
    ...
    $(window).on("scroll", function () {
      ...
      if (scrollHeight - scrollPosition <= footHeight) {
        if (windowWidth >= 768) {
          pageTop.css({
            position: "absolute",
            bottom: "".concat(footHeight + 20, "px") // 768px以上で20px
          });
        } else {
          pageTop.css({
            position: "absolute",
            bottom: "".concat(footHeight + 16, "px") // 768px未満で16px
          });
        }
      } else {
        // 通常位置
  ...
  // モーダル関連の要素を取得 ------------------------------------
  var modalTriggers = $('.js-modal-open img');
  var modal = $('#modal');
  var modalImage = $('.gallery__modal-img');
  var isModalEnabled = $(window).width() > 768; // 初期判定
  ...
  $(function () {
    ...
    function switchTabByHash() {
      var hash = location.hash; //クリックされた#以降の部分を取得
      var $targetTab = $(hash); // id="tab-⚪︎" の要素を取得
    ...
    $tabs.find('a').on("click", function (e) {
      var $link = $(this);
      var link = $link.attr('href'); // リンク先のURLを取得
  ...
  $('.js-accordion-title').on('click', function () {
    var faq = $(this);
    var content = $('#' + faq.attr('aria-controls'));
    var isOpen = faq.attr('aria-expanded') === 'true';
  ...
  // Contactフォーム　------------------------------------
  // 初期状態でボタンを無効化
  var $submitButton = $('#submit-confirm');
  ...
  $submitButton.on('click', function (e) {
    ...
    var $checkboxGroup = $('input[name="services[]"]');
